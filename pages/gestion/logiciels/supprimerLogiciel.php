<!-- Header -->
<?php $nomDeLaPage = basename(__FILE__); include_once('../../insertions/header.php'); ?>

<!-- Body -->
<?php if(isset($_SESSION['mailUser']) || $_SESSION['adminUser'] == 1 ): ?>

    <h1>Supprimer une version </h1>

    <form action="supprimerLogiciel.php" method="POST">

        <div class="champsForm">
            <label for="numVersion" class="labelSelect">Numéro de la version</label>
            <select name="selectVersion" id="numVersion"></select>
        </div>

        <button class="btn-supprimer">Supprimer</button>

    </form>

<?php else: header("Refresh:0; url=../../../index.php"); ?>

<?php endif; ?>

<!-- Footer -->
<?php include_once('../../insertions/footer.php'); ?>